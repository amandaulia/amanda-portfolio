  ...
  return (
    <ol className={`mt-10 grid grid-cols-1 ${colsMap[n]} gap-px bg-rule border border-rule`}>
      {steps.map((s: any, i: number) => (
        <li key={i} className="bg-paper p-5 md:p-6">
...
function SnapshotCase({ c }: any) {
  const n = Math.max(1, Math.min(c.outcome.length, 4));
  const colsMap: any = { 1: "md:grid-cols-1", 2: "md:grid-cols-2", 3: "md:grid-cols-3", 4: "md:grid-cols-4" };
  return (
    <>
      <section className="border-b border-rule">
        <div className="max-w-[1280px] mx-auto px-6 md:px-10 pt-14 md:pt-20 pb-14 md:pb-20">
          <Reveal>
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="eyebrow-sm text-muted">
                Snapshot {c.number} <span className="dot"></span>
                <span className="text-teal">{c.company}</span>
              </div>
              <div className="eyebrow-sm text-muted">{c.role} <span className="dot"></span> {c.period} <span className="dot"></span> {c.geo}</div>
            </div>
          </Reveal>
          <Reveal delay={80}>
            <h1 className="display mt-8 text-[40px] sm:text-[52px] md:text-[64px] lg:text-[76px] leading-[0.98] tracking-[-0.04em]">{c.title}</h1>
          </Reveal>
          <Reveal delay={140}>
            <p className="display display-it mt-7 text-[22px] md:text-[28px] text-ink/85 max-w-3xl leading-[1.25]">{c.hook}</p>
          </Reveal>
          <Reveal delay={200}>
            <div className="mt-8"><TagRow tags={c.tags} lead="ochre" /></div>
          </Reveal>
        </div>
      </section>
      ...
      <section className="border-b border-rule">
        <div className="max-w-[1280px] mx-auto px-6 md:px-10 py-16 md:py-20">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-10">
            <div className="md:col-span-4">
              <Reveal><SectionHead kicker="What I did" title="The work, briefly" /></Reveal>
            </div>
            <div className="md:col-span-8">
              <Reveal delay={80}>
                <p className="display text-2xl md:text-[28px] leading-snug max-w-3xl">{c.what}</p>
              </Reveal>
            </div>
          </div>
        </div>
      ...
      <section className="border-y border-rule grad-signature text-paper">
        <div className="max-w-[1280px] mx-auto px-6 md:px-10 py-20 md:py-28">
          <Reveal>
            <div className="eyebrow text-paper/85">Outcome</div>
            <h2 className="display text-4xl md:text-6xl mt-4 text-paper">The result in numbers</h2>
          </Reveal>
          <div className={`mt-14 grid grid-cols-1 ${colsMap[n]} gap-10 md:gap-12`}>
            {c.outcome.map((m: any, i: number) => (
              <Reveal key={i} delay={i * 80}>
                <div>
                  <div className="metric-num text-paper whitespace-nowrap" style={{ fontSize: "clamp(2.5rem, 4.6vw, 4.5rem)" }}>{m.value}</div>
                  <div className="mt-3 eyebrow text-paper/85">{m.label}</div>
                </div>
              </Reveal>
            ))}
          </div>
...
function CaseStudyPage() {
  ...
  return c.kind === "hero" ? <HeroCase c={c} /> : <SnapshotCase c={c} />;
}
